-- ================================
-- PLOUNIX AUTHENTICATION & PROFILES SETUP (FIXED)
-- ================================

-- NOTE: We don't modify auth.users as it's managed by Supabase
-- Row Level Security is already enabled on auth tables by default
...
-- ================================
-- Note: Vector extension might not be available on free tier

-- Try to enable the vector extension (will fail gracefully if not available)
CREATE EXTENSION IF NOT EXISTS vector;

-- Create table without vector column first (for compatibility)
...
-- Try to add vector column (will fail gracefully if vector extension not available)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector') THEN
    -- Add embedding column only if vector extension is available
    ALTER TABLE public.financial_memories 
...
-- Vector index only if vector extension is available
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector') THEN
...
-- ================================
-- 6. VECTOR SEARCH FUNCTIONS (Optional)
-- ================================
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '⚠️  Vector extension not available (upgrade to Pro for AI embeddings)';
  END IF;
  
  RAISE NOTICE '🎉 Ready to accept user registrations!';
END $$;
