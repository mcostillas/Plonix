...
import { Plus, Receipt, DollarSign, Calendar as CalendarIcon, Tag, CreditCard, FileText } from 'lucide-react'
...
export function EditTransactionModal({ isOpen, onClose, onSuccess, transaction }: EditTransactionModalProps) {
  ...
  useEffect(() => {
    if (isOpen && transaction) {
      ...
      setMonth(transactionDate)
      setDateInputValue(formatDateDisplay(transactionDate))
    }
  }, [isOpen, transaction])
  ...
  const handleSubmit = async () => {
    ...
    if (!user?.id) {
      toast.error('Authentication required', {
        description: 'Please sign in to edit transactions'
      })
      return
    }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto p-3 md:p-6">
        <DialogHeader className="space-y-1 md:space-y-1.5">
          <DialogTitle className="text-base md:text-2xl font-bold">Edit Transaction</DialogTitle>
          ...
          <div className="grid grid-cols-2 gap-2 md:gap-4">
            ...
            >
              <CardHeader className="text-center pb-2 md:pb-3 pt-2 md:pt-4 p-2 md:p-6">
                ...
                <CardTitle className="text-[10px] md:text-lg text-green-600">Income</CardTitle>
                {transactionType === 'income' && (
            ...
            >
              <CardHeader className="text-center pb-2 md:pb-3 pt-2 md:pt-4 p-2 md:p-6">
                ...
                <CardTitle className="text-[10px] md:text-lg text-red-600">Expense</CardTitle>
                {transactionType === 'expense' && (
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-2 md:gap-3 pt-2 md:pt-4">
          ...
          <Button
            ...
            disabled={loading}
            className="w-full sm:w-auto h-8 md:h-10 text-[10px] md:text-sm text-white bg-blue-600 hover:bg-blue-700"
          >
